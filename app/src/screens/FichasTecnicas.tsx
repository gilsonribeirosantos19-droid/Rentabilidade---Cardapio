...
function printFichas(fichas: Ficha[], empresa: Record<string, string> | null, ctx: PrintCtx) {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>Fichas Técnicas</title><style>
    *{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact} body{font-family:Arial,Helvetica,sans-serif;font-size:11px;color:#111;margin:0;background:#e5e7eb} table{width:100%;border-collapse:collapse} .r{text-align:right}
    .pagina{background:#fff;width:210mm;min-height:297mm;margin:14px auto;padding:16mm;box-shadow:0 2px 12px rgba(0,0,0,.25)} .pagina:not(:last-child){page-break-after:always}
    @media print{body{background:#fff} .pagina{width:auto;min-height:0;margin:0;padding:12mm;box-shadow:none}}
        ...
        {tab === 'historico' && (
          <>
            ...
            {hist.length === 0 ? (
              ...
              <table className="vg-tbl" style={{ maxWidth: 760 }}>
                ...
                <tbody>
                  {hist.map((h, i) => {
                    ...
                    return (
                      <tr key={i}>
                        <td style={{ color: '#64748b', fontSize: 11.5 }}>{dt(h.created_at as string)}</td>
                        <td>{ins?.nome || '—'}</td>
