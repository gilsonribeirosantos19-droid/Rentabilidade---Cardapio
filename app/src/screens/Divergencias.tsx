...
type Vinc = { id: string; insumo_id: string; qtd_por_embalagem?: number | null; embalagem_descricao?: string }
type NfeItem = { id: string; descricao_nfe?: string; nfe_id?: string }
type Nfe = { id: string; numero?: string; serie?: string; status?: string; nome_emitente?: string; valor_total?: number; created_at?: string }
...
export function Divergencias() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [insumos, saldos, fichas, vinc, nfeItens, nfeRec, vendas, nfePresas] = await Promise.all([
  ...
  const checks = useMemo<Check[]>(() => {
    if (!data) return []
    const { insumos, saldos, fichas, vinc, nfeItens, nfeRec, vendas, nfePresas } = data
    ...
    const c: Check[] = []
    c.push({ key: 'nfe', titulo: 'Itens de NF-e sem vínculo', cols: ['NF-e', 'Produto da nota'], rows: nfeItens.filter((it) => { const n = recMap[it.nfe_id || '']; return n && n.status !== 'processada' }).map((it) => ({ a: recMap[it.nfe_id || '']?.numero || '—', b: it.descricao_nfe || '—' })) })
